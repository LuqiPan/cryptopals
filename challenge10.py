...
from challenge09 import padding
...
def encrypt_AES_ECB(plain_text, KEY):
    ...
    for i in range(0, len(plain_text), block_size):
        ...
        plain_block = padding(plain_block, block_size)
        cipher_text += cipher_suite.encrypt(plain_block)
    return cipher_text
...
def encrypt_AES_CBC(plain_text, KEY, IV='\x00' * len(KEY)):
    ...
    for i in range(0, len(plain_text), block_size):
        plain_block = plain_text[i:i+block_size]
        plain_block = padding(plain_block, len(KEY))
